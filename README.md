...
coordinator/  Координатор: FastAPI, модели БД, миграции Alembic, API
agent/        Агент: FastAPI-бэкенд (проверка ID и загрузка)
web/          Веб-интерфейс: FastAPI + Jinja2 + HTMX
...
- API координатора — http://localhost:8000 (Swagger: `/docs`)
...
- [ ] Этап 2 — планировщик блоков и протокол аренды (`/lease`, heartbeat, возврат).
- [ ] Этап 3 — агент: двухфазная проверка и загрузка через yt-dlp.
- [ ] Этап 4 — веб: библиотека, добавление загрузок, стрим, живой мониторинг.
- [ ] Этап 5 — rate limiting, восстановление, метрики, безопасность, тесты.
